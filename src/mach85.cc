...
static const char* SysDir_table[3] = {
  "roms85",
  "roms85",
  "roms87"
};

static int RamSizeExt[3]= { 0, 1, 3 };
...
// 'val' is 0-255 if WRITING, <0 || >255 if READING
...
bool HPMachine::processKeyCode(HP85_Key key)
{
  int k = -1;
...
const char* HPMachine::getSysDir() const
{
  return SysDir_table[Model];
}


//****************************************
void HPMachine::HP85OnStartup() {
#if TODO
  long	x, y, w, h, c;
  //	DWORD	cfgrev;
  //	long	hFile;
...
//****************************************
void HPMachine::HP85PWO(bool first) {
#if TODO
	long	i, c;
...
//
void HPMachine::StoreMem(uint16_t addr, uint8_t val)
{
	FixCRTbytes(addr);
...
//
uint8_t HPMachine::LoadMem(uint16_t addr)
{
  uint8_t retval;
  ...
  if (addr>=0177400)  {	// = 0xFF00 I/O
    ...
    if (RamSize<32 && addr>=48*1024) {
      retval = 0xFF;	// if only 16 KB RAM
    }
    else {
      retval = ROM[addr];
    }
...
//************************************************
bool HPMachine::LoadROM(int romnum)
{
	int		f;
        char    filepath[128];
	char	temp[128];

	sprintf(filepath, "%s/rom%3.3o%s", getSysDir(), (unsigned int)romnum, Model==0?"":"B");
        FILE* fh;
	if ( !(fh=getEnvironment()->openEmulatorFile(filepath))) {

          sprintf(filepath, "%s/rom%3.3o", getSysDir(), (unsigned int)romnum);
          if( !(fh=getEnvironment()->openEmulatorFile(filepath))) {
...
//************************************************
bool HPMachine::LoadSYSROM(int sysnum)
{
  char	temp[128], fname[32];
	BYTE	*pBuf;

        const char *suffix = (Model==1) ? "B" : "";
	sprintf(fname, "%s/romsys%d%s", getSysDir(), sysnum, suffix);
	pBuf = ROM + (sysnum-1)*8192;

        FILE* f = getEnvironment()->openEmulatorFile(fname);
	if (!f) {
...
//				while( *pKeyPtr  &&  *pKeyPtr=='\t' ) ++pKeyPtr;
		...
		if ( (CfgFlags & CFG_NATURAL_SPEED) ) {
                  // if running at HP-85 "natural" speed AND we're past the normal startup initialization
			...
			while( Cycles >= CYCLESPERSEC ) {
				...
				StartTime += 1000;
                  ...
                  // if running at fastest possible speed
...
//**************************************************************
//**************************************************************
void HPMachine::Exec85(int num)
{
  uint8_t	opc, msb2, msb3, msb6, savreg, membyt, cy;
  uint16_t	wtmp, xtmp, top, bot, arpreg, pc, pR;
  bool	first;
  uint32_t	dw, ptr, numbytes;
  ...
  while( num-- ) {
    ...
    */
    ...
    if (enableLogging) {
      logging("PC = %06o ", GETREGW(4));
      logging("Op = 0x%x (=%o_8)\n",opc,opc);

      logging("ARP:%o DRP:%o LSB:%d RDZ:%d Z:%d OVF:%d CY:%d DCM:%d LDZ:%d MSB:%d\n", Arp, Drp,
        !!(Flags&LSB), !!(Flags&RDZ), !!(Flags&Z), !!(Flags&OVF), !!(Flags&CY),
        !!(Flags&DCM), !!(Flags&LDZ), !!(Flags&MSB));
      logging("%02x %02x\n",ROM[060000], ROM[060001]);
      for (int i=0;i<64;i++) {
      logging("R%02o:%03o ", i, Reg[i]);
      if ((i%8)==7) logging("\n");
    }
      logging("\n");

      for (int addr=0100202; addr <= 0100241; addr += 8) {
      logging("%06o: ",addr);
      for (int d=0;d<8;d++) {
      logging("%03o ",ROM[addr+d]);
    }
      logging("\n");
    }
    }

    SETREGW(4, GETREGW(4)+1 );   // advance the PC
    ...
    if( msb2==0x00 ) {
      if( (opc & 077)==001 ) {
        // --- ARP 1   load ARP with content of R0
        Arp = Reg[0] & 077;
        Cycles += 3;
      } else {
        // --- ARP AR  load ARP with 6 bit constant
        Arp = opc & 077;
        Cycles += 2;
      }
    }
      ...
      if( (opc & 077)==001 ) {
        // --- DRP 1    load DRP with content of R0
        Drp = Reg[0] & 077;
        Cycles += 3;
      } else {
        // --- DRP DR   load DRP with 6 bit constant
        Drp = opc & 077;
        Cycles += 2;
      }
    }
      ...
      else if( msb3==0xA0 ) {	// LD,ST  opcodes 0240 - 0277
        CPU_101xxxxx_LD_ST(opc);
      }
...
void HPMachine::CPU_101xxxxx_LD_ST(uint8_t opc)
{
  // 101 xxx xx
  //      |  |+-- multi
  //      |  +--- 1: ST / 0: LD
  ...
  //              011: REG_REG_INDIR
  //              100: REG_ADR_DIR
  //              101: REG_REG_ADDR_DIR
  //              110: REG_ADDR_INDIR
  //              111: REG_REG_ADDR_INDIR

  Flags = (Flags & DCM) | Z;

  int top, bot;
  int arpreg, msb3, pR, ptr, numbytes;
  uint16_t wtmp;
  ...
  stldmem:
    ...
    do {
      ++Cycles;
      if( opc & 0x02 ) {
        StoreMem(wtmp, Reg[bot]);	// ST
      }
      else {
        Reg[bot] = LoadMem(wtmp);	// LD
      }
      SetZFlags(bot, first);
      first = false;
      ++bot;
  ...
  rrindir:
    ...
    SETREGW(4, wtmp);
    wtmp -= 2;
    wtmp = (GETROMW(wtmp) + GETREGW(arpreg)) & 0xFFFF;
    SETREGW(2, wtmp);
    Cycles += 2;
    goto stldmem;
...
void HPMachine::CPU_110xxxxx(uint8_t opc)
{
  uint16_t wtmp;
  int top,bot;
  uint8_t membyt;
  bool first;
  uint8_t savreg, cy;
  uint16_t arpreg, pR, xtmp;
  uint8_t msb2,msb3,msb6;
...
void HPMachine::CPU_100xxxxx(uint8_t opc)
{
  uint8_t msb6,msb2,msb3;
  uint16_t wtmp, arpreg, pR;
  int bot,top;
  bool first;
...
//**************************************************************
void HPMachine::SetZFlags(int regnum, bool first) {
...
//**************************************************************
void HPMachine::SetZFlagsR(int regnum, bool first) {
...
uint8_t HPMachine::AddBCD(uint16_t first, uint16_t second, uint8_t *carry) {

  uint16_t	sumlfdig, sumrtdig, locCY;
	...
	sumrtdig = AD_BCD[(sumrtdig<<4) | (uint16_t)(*carry)];
	if( sumrtdig & 0x10 ) {
		locCY = 1;
		sumrtdig &= 0x0F;
	}
	...
	return (uint8_t)((sumlfdig<<4) | sumrtdig);
}
...
uint8_t HPMachine::SubBCD(uint16_t first, uint16_t second, uint8_t *carry) {
  ...
  uint16_t sumlfdig, sumrtdig, locCY;
	...
	sumrtdig = SB_BCD[(sumrtdig<<4) | (1-(uint16_t)(*carry))];
	if( sumrtdig & 0xF0 ) {
		locCY = 0;
		sumrtdig &= 0x0F;
	}
...
//*********************************************************
void HPMachine::SelectROM(uint8_t romnum)
{
  int		i, j;
  uint8_t	curromnum;
...
DWORD	StartTime, HaltTime;
#endif

BOOL  CtlDown=FALSE, ShiftDown=FALSE;
